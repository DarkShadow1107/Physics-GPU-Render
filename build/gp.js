(function (global, factory) {
	...
	var addParticleTorqueToBodyVert =
	...
	var localParticlePositionToRelativeFrag =
	...
	var localParticlePositionToWorldFrag =
	...
	var bodyVelocityToParticleVelocityFrag =
	...
	function World(parameters) {
		...
		Object.defineProperties(this, {
			...
			bodyPositionTexture: {
				get: function () {
					return this.textures.bodyPosRead.texture;
				},
			},
			bodyPositionPrevTexture: {
				get: function () {
					return this.textures.bodyPosWrite.texture;
				},
			},
			bodyQuaternionTexture: {
				get: function () {
					return this.textures.bodyQuatRead.texture;
				},
			},
			bodyQuaternionPrevTexture: {
				get: function () {
					return this.textures.bodyQuatWrite.texture;
				},
			},
			bodyMassTexture: {
				get: function () {
					return this.textures.bodyMass.texture;
				},
			},
			bodyForceTexture: {
				get: function () {
					return this.textures.bodyForce.texture;
				},
			},
			bodyTextureSize: {
				get: function () {
					return this.textures.bodyPosRead.width;
				},
			},
			particlePositionTexture: {
				get: function () {
					return this.textures.particlePosWorld.texture;
				},
			},
			particleLocalPositionTexture: {
				get: function () {
					return this.textures.particlePosLocal.texture;
				},
			},
			particleForceTexture: {
				get: function () {
					return this.textures.particleForce.texture;
				},
			},
			particleTextureSize: {
				get: function () {
					return this.textures.particlePosWorld.width;
				},
			},
			gridTexture: {
				get: function () {
		...
		this.initTextures(parameters.maxBodies || 8, parameters.maxParticles || 8);
	...
	Object.assign(World.prototype, {
		getDefines: function (overrides) {
			...
			var defines = Object.assign({}, overrides || {}, {
				...
				gridResolution:
					"vec3( " +
					gridResolution.x.toFixed(1) +
					", " +
					gridResolution.y.toFixed(1) +
					", " +
					gridResolution.z.toFixed(1) +
					" )",
		...
		getBodyUV: function (bodyId) {
			...
			return new THREE.Vector2(idToX(bodyId, s, s) / s, idToY(bodyId, s, s) / s);
		},
		...
		initTextures: function (maxBodies, maxParticles) {
			...
			Object.assign(this.textures, {
				...
				// Broadphase
				grid: createRenderTarget(
					2 * this.broadphase.resolution.x * this.broadphase.gridZTiling.x,
					2 * this.broadphase.resolution.z * this.broadphase.gridZTiling.y,
					type
				),
			});

			Object.assign(this.dataTextures, {
				bodyPositions: new THREE.DataTexture(
					new Float32Array(4 * bodyTextureSize * bodyTextureSize),
					bodyTextureSize,
					bodyTextureSize,
					THREE.RGBAFormat,
					type
				),
				bodyQuaternions: new THREE.DataTexture(
					new Float32Array(4 * bodyTextureSize * bodyTextureSize),
					bodyTextureSize,
					bodyTextureSize,
					THREE.RGBAFormat,
				...
				particleLocalPositions: new THREE.DataTexture(
					new Float32Array(4 * particleTextureSize * particleTextureSize),
					particleTextureSize,
					particleTextureSize,
					THREE.RGBAFormat,
					type
				),
				bodyMass: new THREE.DataTexture(
					new Float32Array(4 * bodyTextureSize * bodyTextureSize),
					bodyTextureSize,
					bodyTextureSize,
					THREE.RGBAFormat,
		...
		setBodyPositions: function (bodyIds, positions) {
			this.setRenderTargetSubData(
				bodyIds,
				function (out, i) {
					out.set(positions[i].x, positions[i].y, positions[i].z, 1);
				},
				this.textures.bodyPosRead,
				this.textures.bodyPosWrite
			);
		},
		setBodyQuaternions: function (bodyIds, quaternions) {
			this.setRenderTargetSubData(
				bodyIds,
				function (out, i) {
					out.set(quaternions[i].x, quaternions[i].y, quaternions[i].z, quaternions[i].w);
				},
				this.textures.bodyQuatRead,
				this.textures.bodyQuatWrite
			);
		},
		setBodyVelocities: function (bodyIds, velocities) {
			this.setRenderTargetSubData(
				bodyIds,
				function (out, i) {
					out.set(velocities[i].x, velocities[i].y, velocities[i].z, 1);
				},
				this.textures.bodyVelRead,
				this.textures.bodyVelWrite
			);
		},
		setBodyAngularVelocities: function (bodyIds, angularVelocities) {
			this.setRenderTargetSubData(
				bodyIds,
				function (out, i) {
					out.set(angularVelocities[i].x, angularVelocities[i].y, angularVelocities[i].z, 1);
				},
				this.textures.bodyAngularVelRead,
				this.textures.bodyAngularVelWrite
			);
		},
				...
				for (var i = 0; i < gridTexture.width / 2; i++) {
					for (var j = 0; j < gridTexture.height / 2; j++) {
						onePointPerTexelGeometry.vertices.push(
							new THREE.Vector3((2 * i) / (gridTexture.width / 2) - 1, (2 * j) / (gridTexture.height / 2) - 1, 0)
						);
					}
				}
				setGridStencilMaterial = this.materials.setGridStencil = new THREE.PointsMaterial({
					size: 1,
		...
		updateBodyForce: function () {
			...
			if (!addForceToBodyMaterial) {
				...
				mapParticleToBodyGeometry.addAttribute(
					"position",
					new THREE.BufferAttribute(new Float32Array(numParticles * numParticles * 3), 3)
				);
