(function (global, factory) {
	typeof exports === "object" && typeof module !== "undefined"
		...
		: (global.Demo = factory(global.THREE));
})(this, function (THREE) {
	"use strict";
	...
	function Demo(parameters) {
		...
		function init() {
			...
			if (parameters.cameraPosition) camera.position.copy(parameters.cameraPosition);
			else camera.position.set(0, 0.6, 1.4);
			...
			var checkerTexture = new THREE.DataTexture(
				new Uint8Array([255, 0, 0, 255, 255, 255, 255, 255]),
				2,
				1,
				THREE.RGBAFormat,
			...
			customDepthMaterial = new THREE.ShaderMaterial({
				uniforms: THREE.UniformsUtils.merge([THREE.ShaderLib.depth.uniforms, meshUniforms]),
			...
			// interaction
			interactionSphereMesh = new THREE.Mesh(
		...
		function render() {
			...
			customDepthMaterial.uniforms.particleLocalPosTex.value = debugMesh.material.uniforms.particleLocalPosTex.value =
				world.particleLocalPositionTexture;
			customDepthMaterial.uniforms.posTex.value = debugMesh.material.uniforms.posTex.value = world.bodyPositionTexture;
			customDepthMaterial.uniforms.posTexPrev.value = debugMesh.material.uniforms.posTexPrev.value =
				world.bodyPositionPrevTexture;
			customDepthMaterial.uniforms.quatTex.value = debugMesh.material.uniforms.quatTex.value = world.bodyQuaternionTexture;
			customDepthMaterial.uniforms.quatTexPrev.value = debugMesh.material.uniforms.quatTexPrev.value =
				world.bodyQuaternionPrevTexture;
			customDepthMaterial.uniforms.interpolationValue.value = debugMesh.material.uniforms.interpolationValue.value =
				world.interpolationValue;
		...
		function initGUI() {
			controller = {
				moreObjects: function () {
					location.href = "?n=" + numParticles * 2;
				},
				lessObjects: function () {
					location.href = "?n=" + Math.max(2, numParticles / 2);
				},
				paused: false,
